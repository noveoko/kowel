...
class ImageAlignerGUI:
    def __init__(self, root):
        ...
        if not os.path.exists(self.align_tool):
            messagebox.showerror("Error", "align_image_stack.exe not found!\nPlease install Hugin first.")
            root.quit()
    ...
    def create_widgets(self):
        ...
        ttk.Entry(input_frame, textvariable=self.input_dir, width=50).pack(side="left", padx=5)
        ...
        ttk.Checkbutton(options_frame, text="Use GPU acceleration", variable=self.use_gpu).pack(anchor="w")
        
        self.auto_crop = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Auto-crop aligned images", variable=self.auto_crop).pack(anchor="w")
        ...
        ttk.Button(button_frame, text="Cancel", command=self.cancel_alignment).pack(side="left")
    ...
    def align_images(self):
        ...
        cmd = [
            self.align_tool,
            '-a', str(output_path / 'aligned_'),  # output prefix
        ...
        try:
            ...
            if rc == 0:
                ...
                messagebox.showinfo("Success", "Image alignment completed successfully!")
            else:
